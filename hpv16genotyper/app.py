...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.binaries = appFunctions._init_binaries(
            sys.platform
        ...
        self.blastTimer.timeout.connect(
            lambda: appFunctions.MainFunctions.blastProgress(self.cliMainFunctions)
        )
        self.cliMainFunctions.startTimerSignal.connect(
            lambda: self.blastTimer.start(1000)
        )
        ...
        self.cliMainFunctions.errorSignal.connect(
            lambda x: GuiFunctions.showError(self, x)
        )
        self.show()
...
class GuiFunctions(MainWindow):
    ...
    def addHomeButtons(self):
        ...
        self.homeButtonNames = [
            "Load the fasta file to analyze",
            "Select directory to write the output",
    ...
    def getFastaFile(self: QMainWindow) -> None:
        ...
        if fasta_path != pathlib.Path("."):
            ...
            mainW.paramsdf.loc["out"] = mainW.paramsdf.loc["out"] / pathlib.Path(
                str(fasta_path.stem)
                + "_"
    ...
    def loadResDir(self) -> None:
        ...
        if response == "":
            ...
            if mainW.outdir != mainW.fasta_path.parent:
                mainW.paramsdf.loc["query", "Value"] = (
                    mainW.outdir / mainW.fasta_path.name
                )
                mainW.fasta_path = mainW.paramsdf.loc["query", "Value"]
    ...
    def initVariables(self: QMainWindow) -> None:
        ...
        self.dfBlast = pd.read_excel(
            self.outdir / "GeneIdentification_results.xlsx", engine="openpyxl"
        )
        self.dfLineageSnp = pd.read_excel(
            self.outdir / "LineageSpecificSNPs.xlsx", engine="openpyxl", index_col=0
        )
        self.dfCancerSnp = pd.read_excel(
            self.outdir / "cancerSNP_results.xlsx", engine="openpyxl"
        )
        self.dfGeneIdentification = pd.read_excel(
            self.outdir / "GeneIdentification_results.xlsx",
            engine="openpyxl",
            index_col=0,
        )
        ...
        if num_of_nonHPV16 == 1:
            ...
            nonHPV16Msg = (
                f"{num_of_nonHPV16} non-HPV16 sequences have been found and removed"
            )
        if num_of_recombinants == 1:
            recMsg = "1 putative recombinant sequence has been found"
        else:
            recMsg = (
                f"{num_of_recombinants} putative recombinant sequences have been found"
            )
    ...
    def updateResultsTables(self) -> None:
        ...
        tables = [
            mainW.ui.blastResTable,
            mainW.ui.lineageSnpTable,
            mainW.ui.lineageSumsTable,
            mainW.ui.cancerSnpTable,
        ]
        ...
        for idx in indeces:
            ...
            if tableName == "blastResTable":
                if mainW.dfBlast.size == 0:
                    return
                tmpdf = mainW.dfBlast[
                    mainW.dfBlast["Query sequence"] == mainW.selectedSeq
                ]
                tmpdf = tmpdf.drop("Query sequence", axis=1)
                tmpdf.sort_values("Query start", inplace=True)
                tmpdf["Perc. identity"] = tmpdf["Perc. identity"].apply(
                    lambda x: "{:0.2f}".format(x)
                )
                tmpdf["E-value"] = tmpdf["E-value"].apply(lambda x: f"{x:.2e}")
            if tableName == "lineageSnpTable":
                if mainW.dfLineageSnp.size == 0:
                    return
                tmpdf = (
                    mainW.dfLineageSnp[mainW.dfLineageSnp.index == mainW.selectedSeq]
                    .tail(1)
                    .T.head(67)
                )  # 67 == num_snps
            ...
            if tableName == "cancerSnpTable":
                if mainW.dfCancerSnp.size == 0:
                    return
                tmpdf = mainW.dfCancerSnp[
                    mainW.dfCancerSnp["Query sequence"] == mainW.selectedSeq
                ]
                tmpdf.sort_values(
                    by=["Query position"], inplace=True, ignore_index=True
                )
            ...
            if tableName != "lineageSnpTable":
                table.horizontalHeader().setDefaultSectionSize(
                    int(1100 / tmpdf.shape[1])
                )
            if tableName == "lineageSumsTable":
                table.horizontalHeader().setDefaultSectionSize(120)
            if tableName == "cancerSnpTable":
                table.horizontalHeader().setDefaultSectionSize(
                    int(900 / tmpdf.shape[1])
                )
        return
    ...
    def displayGraphs(self: QListWidgetItem) -> None:
        ...
        for browser in browsers:
            ...
            if browserName == "geneIDBrowser":
                tmpdf = mainW.dfGeneIdentification[
                    mainW.dfGeneIdentification.index == mainW.selectedSeq
                ]
            ...
            if browserName == "lineageSnpBrowser":
                tmpdf = mainW.dfLineageSnp.loc[
                    (mainW.dfLineageSnp.index == mainW.selectedSeq)
                ]
                tmpdf = tmpdf.T.head(67)  # num of lineage specific snps
                tmpdf.columns = ["Lineage"]
                tmpdf["SNP reference pos"] = tmpdf.index
                tmpdf["SNP reference pos"] = tmpdf["SNP reference pos"].apply(
    ...
    def displayCancerSnpInfo(self: QTableWidgetItem) -> None:
        ...
        cSnp = cSnp + ".md"
        mainW.ui.cancerSnpTextBrowser.setSearchPaths(
            [
                str(
    ...
    def createSimplot(self) -> None:
        ...
        mainW.aln_f = simplot.align(
            mainW.muscle_bin,
            mainW.simplotDBFasta,
            mainW.totalSequenceRecDict,
            mainW.selectedSeq,
            mainW.tmpOut,
        )
        ...
        mainW.simplotW = Simplot_page(
            mainW.selectedSeq, mainW.window_size, mainW.step, mainW.aln_f
        )
        mainW.simplotW.show()
        return
...
class Simplot_page(QMainWindow):
    def __init__(self, sequence, window_size, step, aln_f, parent=None):
        ...
        self.viewbox = pg.ViewBox(
            enableMenu=False,
            enableMouse=False,
            border=pg.mkPen(color=(0, 0, 0), width=0.5),
        )
        ...
        self.viewbox_label = pg.LabelItem(
            '<span style="color:black;font-size:11pt">Legend: </span>',
            parent=self.viewbox,
        )
        ...
        # Plot the curves
        self.xaxisData, self.yaxisData, self.alnSize, self.qseqSize = (
            simplot.calculate_similarities(self.qseq, self.aln_f, self.step, self.wsize)
        )
        self.c1 = self.plot.plot(
            self.xaxisData,
            self.yaxisData["NC_001526.4_A1"],
            name="Lineage A",
            pen=self.pen_green,
        )
        self.c2 = self.plot.plot(
            self.xaxisData,
            self.yaxisData["AF536180.1_B1"],
            name="Lineage B",
            pen=self.pen_blue,
        )
        self.c3 = self.plot.plot(
            self.xaxisData,
            self.yaxisData["AF472509.1_C1"],
            name="Lineage C",
            pen=self.pen_orange,
        )
        self.c4 = self.plot.plot(
            self.xaxisData,
            self.yaxisData["HQ644257.1_D1"],
            name="Lineage D",
            pen=self.pen_red,
        )
        ...
        # Modify plot
        self.title = (
            '<span style="color:black;font-size:11pt;background-color:#cffaf9">Sequence: %s Sequence size: %s Alignment size: %s Window: %s Step: %s</span>'
...
class Ui_MainWindow(QMainWindow):
    def setupUi(self, MainWindow):
        ...
        self.pageNameInfo.setStyleSheet("color: rgb(98, 103, 111);")
        ...
        self.frameContentRightPanel.setStyleSheet("background-color: rgb(44, 49, 60);")
        ...
        self.statusLines = [
            "HPV-16 filtering",
            "Gene Identification",
            "Lineage specific SNPs scan",
            "Cancer specific SNPs scan",
            "Gene alignment",
            "Trees calculation",
        ]
        ...
        self.putativeRecombinants = (
            QCheckBox()
        )  # Will use this to "transform" the list into a putative recombinants only
        ...
        self.blastResVerticalLayout.setObjectName("blastResVerticalLayout")
        ...
        # 3. LED
        self._blastLED = QLed(
            self, onColour=QLed.Green, offColour=QLed.Red, shape=QLed.Circle
        )
        ...
        self.lineageSnpVerticalLayout.setObjectName("lineageSnpVerticalLayout")
        ...
        self.TreesRenderButtonGroup.buttonClicked[int].connect(
            GuiFunctions.eteInteractive
        )
        row = 1
        col = 0
        self.genes = ["E6", "E7", "E1", "E2", "E4", "E5", "L2", "L1"]
        ...
        self.simplotButtonAreaWidgetLayout.addWidget(
            self.simplotButtonAreaWidgetSimplotLabel, 0, 0, 1, 2
        )
        self.simplotButtonAreaWidgetLayout.addWidget(
            self.simplotButtonAreaWidgetWindowLabel, 1, 1
        )
        self.simplotButtonAreaWidgetLayout.addWidget(
            self.simplotButtonAreaWidgetStepLabel, 2, 1
        )
        ...
        self.resultsPageGridLayoutForFrames.addWidget(
            self.saveGraphicsFrame, 7, 5, 1, 1
        )
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.menuLabel.setText(
            QCoreApplication.translate(
                "MainWindow", "Please use the following menu", None
            )
        )
