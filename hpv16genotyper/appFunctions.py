...
class MainFunctions(QObject):
    ...
    def create_dirs(self, outdir: pathlib.Path, exist_ok: bool = True) -> pathlib.Path:
        ...
        profiles_out = script_out / pathlib.Path(
            "Alignments"
        )  # Final profile alns for each gene
    ...
    def filter_hpv16(
        self, fasta_file_path: pathlib.Path, seqs_to_include: list, outdir: pathlib.Path
    ) -> str:
        """
        Write the filtered output file in the output directroy with name: Filtered_" + fasta_file_path.name
        """

        def _remove_problematic_seqs(
            fasta_file_path: pathlib.Path, seqs_to_include: list
        ) -> dict:
            """
    ...
    def blastProgress(self):
        ...
        def _helper(f):
            ...
            if f.exists():
                df_readCSVPartial = pd.read_csv(
                    f, sep="\t", names=column_names, usecols=["qaccver"]
                )  # Read partial data for lower memory consuption
                df_readCSVPartial["qaccver"] = df_readCSVPartial["qaccver"].astype(
                    "category"
                )  # Lower memory consuption for large datasets
    ...
    def blastn_search(
        self,
        exe_params: pd.DataFrame,
        query_f_path: pathlib.Path,
        workflow: str,
        makeblastdb_bin: str,
        blastn_bin: str,
        total: int,
    ...
    def parse_GeneID_results(
        ...
        """
        geneID_results_path = (
            blastres_f_path.parent / ".." / "GeneIdentification_results.xlsx"
        )
        if not exe:
        ...
        df["Subject coverage"] = (
            (df["Subject end"] - df["Subject start"] + 1) / df["Subject length"]
        ) * 100
        ...
        df["Sublineage"] = t["Subject sequence"].transform(
            lambda x: _filterSublineage(x)
        )
        df["Subject sequence"] = t["Subject sequence"].transform(
            lambda x: _filterLineage(x)
        )
    ...
    def parse_SNP_results(
        self,
        blastres_f_path: pathlib.Path,
        seqindex: dict,
        exe: bool = True,
        cancer: bool = False,
    ) -> pathlib.Path:
        ...
        if cancer:
            probe_results_path = (
                blastres_f_path.parent / ".." / "cancerSNP_results.xlsx"
            )
            logging.info("Parsing - increased cancer risk SNP results ")
        else:
            probe_results_path = (
                blastres_f_path.parent / ".." / "LineageSpecificSNPs.xlsx"
            )
        ...
        def _get_aln_nucl(qseq, qstart, sstart, probe_len) -> list:
            ...
            return [
                qseq[qstart + middle_pos - 1],
                middle_pos,
            ]  #  - 1 because the sequence is 0 based
        ...
        for idx in indeces:
            ...
            if cancer:
                if evalue < probe_table_res[qorg][subjct_probe]["E-value"]:
                    ...
                    probe_table_res[qorg][subjct_probe]["E-value"] = float(
                        "{:0.3e}".format(evalue)
                    )
            else:
        ...
        if cancer:
            ...
            probedf["Cancer SNP"] = probedf["Reference genome position"].apply(
                lambda x: _cancerSNP(x)
            )
            probedf["Reference genome position"] = probedf[
            ...
            for qorg in probe_table_res:
                for subjct_probe in probe_table_res[qorg]:
                    probe_table_res[qorg][subjct_probe] = probe_table_res[qorg][
                        subjct_probe
                    ][0]
    ...
    def identify_nonHPV16(self, blastres_f_path: pathlib.Path) -> list:
        ...
        for seq in seqs:
            tmpdf = df.loc[
                (df["Query sequence"] == seq)
                & (df["Perc. identity"] >= 90.00)
                & (df["Query coverage"] >= 78)
                & (df["Subject sequence"] == "HPV16_cg")
            ]  # 78% was used in our relaxed analysis (45 recombination events), to allow many Ns
            if not tmpdf.empty:
                evalue = 1
                final_idx = None
                indeces = tmpdf.index
        ...
        logging.info(
            f"Finished, identified {len(hpv16_sequences)} HPV16 and {len(nonHPV16Seqs)} non HPV16 sequences "
        )
        return hpv16_sequences

    def annotate_results(
        self,
        annot_f: pathlib.Path,
        probe_res: pathlib.Path,
        empty: str = "Other",
        exe: bool = True,
    ) -> int:
        """
        ...
        for col in annotdf.columns:
            for idx in annotdf.index:
                if col not in extra_row["Nucleotides"]:
                    ...
                    extra_row["Nucleotides"][col] += (
                        "," + idx + ":" + annotdf.loc[idx, col]
                    )
    ...
    def find_recombinants(
        self, blastdf: pd.DataFrame, lineageSnpDf: pd.DataFrame, outdir: pathlib.Path
    ) -> dict:
        """
        ...
        recombinants = [
            org
            for org in recombStatus
            if recombStatus[org]["geneID"] != 0 and recombStatus[org]["lSNP"] != 0
        ]
    ...
    def prepare_alns(
        ...
        outdir: pathlib.Path,
        query_f_path: pathlib.Path,
        geneid_blast_res_xls: pathlib.Path,
        muscle_bin: str,
        profiledb_dir: pathlib.Path,
        threads: int,
    ) -> list:
        """
    ...
    def main(self, paramsdf: pd.DataFrame, exe: bool = True) -> pathlib.Path:
        ...
        hpv16filt_results = self.blastn_search(
            paramsdf,
            query_f_path,
            workflow,
            makeblastdb_bin,
            blastn_bin,
            total=query_orgs_num,
        )
        ...
        geneid_blastn_res_path = self.blastn_search(
            paramsdf,
            query_f_path,
            workflow,
            makeblastdb_bin,
        ...
        geneid_blastn_res_path_xlsx, blastDF = self.parse_GeneID_results(
            geneid_blastn_res_path, paramsdf, exe=exe
        )
        ...
        workflow = "cancer"
        C_snp_blastn_res_path = self.blastn_search(
            paramsdf,
            query_f_path,
            workflow,
            makeblastdb_bin,
...
def _init_binaries(system: sys.platform) -> list:
    """Resolve the external executables used by the workflow.

    Development installs use the user's PATH.  Frozen applications keep the
    ...
    if meipass:
        ...
        binaries = [bin_dir / f"{name}{executable_suffix}" for name in binary_names]
        missing = [path for path in binaries if not path.is_file()]
        if missing:
            missing_names = ", ".join(path.name for path in missing)
            raise FileNotFoundError(
                f"Missing bundled executable(s) in {bin_dir}: {missing_names}"
            )
        return [str(path) for path in binaries]
    ...
    for name in binary_names:
        ...
        if resolved is None:
            ...
            binaries.append(str(pathlib.Path(resolved)))
    if missing:
        raise FileNotFoundError(
...
def _defaultparams() -> dict:
    ...
    input_dir = ""
    query_f = ""
    outdir = pathlib.Path(__file__).absolute().parent / pathlib.Path(
        "Results_directory"
    )
    ...
    params = {
        ...
        "SNP_annotation_file": pathlib.Path(__file__).parent
        / "resources"
        / "SNPs_annotation.xlsx",
        "SNP_db_path": pathlib.Path(__file__).parent
        / "resources"
        / "sequences"
        / "NC_001526_probes.fa",
        "cSNP_db_path": pathlib.Path(__file__).parent
        / "resources"
        / "sequences"
        / "NC_001526_cancer_probes.fa",
        "Gene_identification_Evalue": 1e-5,
        "Gene_identification_Word_size": 7,
        "GenesProfile_directory": pathlib.Path(__file__).parent
        / "resources"
        / "sequences"
        / "profiles",
        "GenesRef_database": pathlib.Path(__file__).parent
        / "resources"
        / "sequences"
        / "16refs_gene_db.fa",
        "SimplotRef_database": pathlib.Path(__file__).parent
        / "resources"
        / "sequences"
        / "Reference_genomes_profile_mafft_GINSI.fa",
        "HPV_filter_db": pathlib.Path(__file__).parent
        / "resources"
        / "sequences"
        / "alphapapillomavirus9.fa",
    }
    return params
